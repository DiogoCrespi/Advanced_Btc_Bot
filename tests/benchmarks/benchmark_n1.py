...
from MiroFish.backend.scripts.run_parallel_simulation import fetch_new_actions_from_db
...
def create_mock_db(db_path, num_actions=1000):
    ...
    for i in range(1, 1001):
        cursor.execute("INSERT INTO post VALUES (?, ?, ?, ?, ?)", (i, random.randint(1, 100), f"Post {i} content", None, f"Quote {i} content"))
        cursor.execute("INSERT INTO comment VALUES (?, ?, ?, ?)", (i, random.randint(1, 1000), random.randint(1, 100), f"Comment {i} content"))
        cursor.execute("INSERT INTO follow VALUES (?, ?, ?)", (i, random.randint(1, 100), random.randint(1, 100)))
    ...
    for i in range(num_actions):
        ...
        if action in ('like_post', 'dislike_post', 'create_comment'):
            info['post_id'] = random.randint(1, 1000)
        elif action == 'repost':
            info['new_post_id'] = random.randint(1, 1000)
        elif action == 'quote_post':
            info['quoted_id'] = random.randint(1, 1000)
            info['new_post_id'] = random.randint(1, 1000)
        elif action == 'follow':
            info['follow_id'] = random.randint(1, 1000)
        elif action == 'mute':
            info['user_id'] = random.randint(1, 100)
        elif action in ('like_comment', 'dislike_comment'):
            info['comment_id'] = random.randint(1, 1000)

        cursor.execute("INSERT INTO trace VALUES (?, ?, ?, ?)", (random.randint(1, 100), action, json.dumps(info), "2023-01-01"))
...
if __name__ == "__main__":
    ...
    create_mock_db(db_path, 5000)

    agent_names = {i: f"AgentName_{i}" for i in range(1, 101)}
