...
def test_real_opportunity_breakaway():
    """
    TESTE DE OPORTUNIDADE: Gap de rompimento com volume alto e confluência.
    Deve ser classificado como oportunidade real (conviction >= 0.7).
    """
    logic = GapLogic()
    
    # 30 períodos de consolidação
    df = pd.DataFrame({
        'open':   [100.0]*30,
        'close':  [100.0]*30,
        'high':   [101.0]*30,
        'low':    [99.0]*30,
        'volume': [100.0]*30,
        'SMA_50': [100.0]*30 # SMA_50 estável
    })
    # Injecao de um Breakaway Gap no índice 30
    new_row = pd.DataFrame({
        ...
        'cvd_div': [1]     # Divergência CVD positiva (Buyer absorption)
    })
...
def test_exhaustion_risk():
    """
    TESTE DE RISCO: Gap no final de uma tendência esticada.
    Deve ser classificado como Exaustão e NÃO ser uma oportunidade real.
    """
    logic = GapLogic()
    # Tendência de alta esticada (>5% em 10 candles)
    ...
    # Gap Final de Exaustão
    new_row = pd.DataFrame({
        'open': [125.0],
        'close': [126.0],
        'high': [128.0],
        'low': [124.0],
        'volume': [600.0], # Volume altíssimo
        'SMA_50': [121.0],
        'cvd_div': [0]
    })
    df = pd.concat([df, new_row]).reset_index(drop=True)
    ...
    assert is_opp == False
    assert conviction < 0.5 # Penalização por exaustão deve baixar a convicção

def test_common_gap_no_conviction():
    """
    TESTE NEUTRO: Gap comum sem volume ou contexto.
    Deve ter baixa convicção e não ser uma oportunidade.
    """
    logic = GapLogic()
    df = pd.DataFrame({
        'open':   [100.0]*30,
