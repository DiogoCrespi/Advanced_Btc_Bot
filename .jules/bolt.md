...
## 2024-05-24 - Replacing loss.replace() and Pandas division with np.where() and np.errstate()
**Learning:** Found a major performance bottleneck where `loss.replace(0, np.nan)` and scalar Pandas division were used in RSI calculations (`data/data_engine.py`, `logic/usdt_brl_logic.py`, `logic/xaut_logic.py`). Pandas scalar-friendly methods and series-level operations for conditions and replacement are significantly slower than underlying numpy operations.
**Action:** Extract `.values` for series like `gain` and `loss`. Use `with np.errstate(divide='ignore', invalid='ignore'):` to suppress warnings, perform standard numpy division `rs = gain / loss`, and correctly handle zeros via `np.where(loss == 0, np.nan, rs)`. This safely avoids index alignment overhead and propagates mathematical correctness, speeding up calculations natively.
