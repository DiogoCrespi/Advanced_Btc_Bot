...
## 2026-04-03 - Avoid Pandas Series Instantiation Overhead
**Learning:** Using `df.iloc[-1]` to extract a row creates a new `pd.Series` object which is computationally expensive and introduces significant overhead in high-frequency trading loops.
**Action:** Always access the underlying NumPy arrays directly using `df['column'].values[-1]` when extracting scalar values, and ensure to explicitly cast the result to a native Python type (like `float()`) to prevent JSON serialization errors downstream.
