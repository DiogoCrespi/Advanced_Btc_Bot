...
## 2024-05-24 - Replacing python loops with np.maximum.accumulate for Max Drawdown
**Learning:** Found a performance bottleneck in `tools/optimizer.py` where a Python `for` loop was used to iterate over the `pnl_history` array (which can contain up to 100k+ elements) to calculate the maximum drawdown. Iterating in pure Python is O(N) and slow (~0.066s per run). By replacing the loop with `np.maximum.accumulate(pnls)` to track the peak equity array, the drawdown calculation is completely vectorized in C, reducing the time to ~0.002s (a ~30x speedup).
**Action:** Always prefer `np.maximum.accumulate()` combined with vectorized array division instead of Python `for` loops when calculating Maximum Drawdown or tracking running maximums over large history arrays.
