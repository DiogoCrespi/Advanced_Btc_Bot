...
## 2024-04-30 - [Pandas Vectorized Type Conversion]
**Learning:** In pandas, replacing `df[cols].apply(pd.to_numeric)` with `df[cols].astype(float)` yields substantial performance improvements during data ingestion by avoiding Python-level loop overhead in favor of C-level vectorization.
**Action:** Always prefer `.astype()` over `.apply()` for type conversions across multiple dataframe columns when error handling requirements permit it.
