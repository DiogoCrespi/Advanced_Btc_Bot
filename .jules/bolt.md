...
## 2024-05-24 - Replacing apply(pd.to_numeric) with astype(float) for block type conversion
**Learning:** Found a major performance bottleneck where `df[cols].apply(pd.to_numeric)` was used to convert incoming text data blocks (klines) to numeric representations in `data/data_engine.py`. The `.apply()` method introduces slow Python-level loop overhead over columns. Converting the columns as a block using `df[cols] = df[cols].astype(float)` achieves a measurable speedup (e.g. ~6x faster in local tests) while identically replacing string-to-float inferences in our use-case.
**Action:** Always prefer `astype(float)` for vectorized block type conversion of known numeric string columns instead of `apply(pd.to_numeric)` to bypass Python-level overhead during data ingestion and API consumption.
