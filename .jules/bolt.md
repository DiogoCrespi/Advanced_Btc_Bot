...
## 2024-03-24 - Pandas Scalar Methods Performance in RSI Calculations
**Learning:** Using Pandas scalar-friendly methods like `pd.Series.replace(0, np.nan)` or `.where(condition, 0.0)` for condition clamping and division-by-zero protection in rolling calculations is a significant performance bottleneck compared to NumPy operations.
**Action:** Always extract values to NumPy arrays (`.values`), use `np.maximum()` for clamping, and `np.where()` wrapped in `np.errstate` for division-by-zero protection before wrapping back to Pandas series for `.rolling` execution. This yields a measurable ~1.3x speedup.
