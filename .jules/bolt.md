...
## 2024-05-24 - Replacing Pandas DatetimeIndex properties with NumPy arrays for Temporal Encoding
**Learning:** Found a major performance bottleneck where sequential math operations (like trigonometric temporal encodings `np.sin()` and `np.cos()`) were executed directly on Pandas DatetimeIndex properties (e.g., `df.index.hour`). Pandas index-alignment and Series creation incur substantial overhead.
**Action:** Extract values to underlying NumPy arrays first (e.g., `df.index.hour.values`) and pre-calculate constants before performing array operations to eliminate overhead and improve performance.
