    ...
    def run(self):
        ...
        def get_ext_filename(ext):
            ...
            if self.inplace:
                ...
                return os.path.join(package_dir, self.get_ext_filename(base))
            else:
                return self.get_ext_fullpath(ext.name)

        pkg_roots = {}
        for ext in self.extensions:
            pkg_root = self.get_ext_fullname(ext.name).split('.')[0]
            pkg_roots.setdefault(pkg_root, []).append(ext)

        for pkg_root, exts in pkg_roots.items():
            shared_lib_dir = os.path.join(pkg_root, '.libs')
            if not self.inplace:
                shared_lib_dir = os.path.join(self.build_lib, shared_lib_dir)

            available_dlls = [fn for fn in os.listdir(self.extra_dll_dir) if fn.lower().endswith('.dll')]
            required_dlls = set()
            files_to_scan = [get_ext_filename(ext) for ext in exts]
            ...
            while files_to_scan:
                ...
                for dll in available_dlls:
                    if dll in required_dlls:
                        continue
                    if dll.lower().encode('ascii') in data:
                        required_dlls.add(dll)
                        files_to_scan.append(os.path.join(self.extra_dll_dir, dll))

            for fn in required_dlls:
