    ...
    def _compile(self, obj, src, ext, cc_args, extra_postargs, pp_opts):
        ...
        if ext in ['.F', '.F90', '.FOR']:
            if self.c_compiler is None:
                raise CompileError("C compiler is needed for preprocessing Fortran source %s" % src)
            processed_ext = '.f90' if ext == '.F90' else '.f'
            processed_src = os.path.splitext(obj)[0] + processed_ext
            self.mkpath(os.path.dirname(processed_src))
            self.c_compiler.preprocess(src, output_file=processed_src,
                                       extra_preargs=pp_opts)
            src = processed_src
            ext = processed_ext
...
def get_f90flags(src):
    """
    Search the first 20 lines of fortran 90 code for line pattern
      `!F90FLAGS(<fcompiler type>)=<f90 flags>`
    Return a dictionary {<fcompiler type>:<f90 flags>}.
    """
    flags = {}
    with open(src, encoding='latin1') as f:
        i = 0
