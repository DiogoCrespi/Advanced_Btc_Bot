...
class GapLogic:
    ...
    def classify_gap(self, df, row_idx):
        ...
        row_open = df['open'].values[row_idx]
        prev_close = df['close'].values[row_idx-1]
        gap_size = abs(row_open / prev_close - 1)
        
        if gap_size < 0.005: return "Normal"
        
        avg_vol = df['volume'].values[max(0, row_idx-20):row_idx].mean()
        row_vol = df['volume'].values[row_idx]
        is_high_vol = row_vol > avg_vol * 1.5
        
        # Trend detection
        prev_idx_10 = max(0, row_idx-10)
        trend = (prev_close / df['close'].values[prev_idx_10]) - 1
    ...
    def evaluate_opportunity(self, df, row_idx):
        ...
        row_open = df['open'].values[row_idx]
        prev_close = df['close'].values[row_idx-1]
        gap_size = (row_open / prev_close) - 1
        ...
        avg_vol = df['volume'].values[max(0, row_idx-20):row_idx].mean()
        row_vol = df['volume'].values[row_idx]
        vol_boost = min(1.0, (row_vol / avg_vol) / 4.0) # Cap at 4x Avg
        ...
        if 'SMA_50' in df.columns:
            sma_slope = df['SMA_50'].diff(5).values[row_idx]
