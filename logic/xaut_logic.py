...
class XAUTAnalyzer:
    ...
    def get_signal(self, df: pd.DataFrame) -> tuple[int, float, str]:
        ...
        rsi    = float(df['ratio_rsi'].to_numpy()[-1]) if 'ratio_rsi' in df.columns else 50.0
        bb_pct = float(df['bb_pct'].to_numpy()[-1]) if 'bb_pct' in df.columns else 0.5
        slope  = float(df['ratio_slope'].to_numpy()[-1]) if 'ratio_slope' in df.columns else 0.0
