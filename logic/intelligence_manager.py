...
class IntelligenceManager:
    ...
    def fetch_macro_data(self):
        ...
        try:
            ...
            data = {
                "dxy_close": float(dxy['Close'].to_numpy()[-1]),
                "dxy_change": (float(dxy['Close'].to_numpy()[-1]) / float(dxy['Close'].to_numpy()[-2])) - 1,
                "gold_close": float(gold['Close'].to_numpy()[-1]),
                "gold_change": (float(gold['Close'].to_numpy()[-1]) / float(gold['Close'].to_numpy()[-2])) - 1,
                "sp500_close": float(sp500['Close'].to_numpy()[-1]),
