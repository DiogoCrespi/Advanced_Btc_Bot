    ...
    def fetch_macro_data(self):
        ...
        try:
            ...
            data = {
                ...
                "dxy_change": (float(dxy['Close'].values[-1]) / float(dxy['Close'].values[-2])) - 1,
                "gold_close": float(gold['Close'].values[-1]),
                "gold_change": (float(gold['Close'].values[-1]) / float(gold['Close'].values[-2])) - 1,
                "sp500_close": float(sp500['Close'].values[-1]),
                "sp500_change": (float(sp500['Close'].values[-1]) / float(sp500['Close'].values[-2])) - 1,
